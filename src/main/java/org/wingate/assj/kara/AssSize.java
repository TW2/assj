...
public class AssSize {
    ...
    public static AssSize create(ASS ass, AssEvent event){
        AssSize az = new AssSize();
        Pattern p; Matcher m;
        String text = event.getText().replace("}{", "");
        
        // Extract overrides
        p = Pattern.compile("\\{(?<overrides>)[^\\}]?\\}");
        m = p.matcher(text);        
        String tags = "";
        if(m.find()){
            tags = m.group("overrides");
        }
        ...
        az.font = new Font(fontName, (bold ? Font.BOLD : 0) + (italic ? Font.ITALIC : 0), (int)fontSize).deriveFont(fontSize);
        
        // Mesure
        FontMetrics fm = g2d.getFontMetrics(az.font);

        // Fausse taille :
        float falseSizeInPoints = az.font.getSize2D();
        ...
        az.font = new Font(fontName, (bold ? Font.BOLD : 0) + (italic ? Font.ITALIC : 0), (int)trueSizeInPoints)
        ...
        az.lineWidth = fm.stringWidth(strippedline);
        az.lineHeight = fm.getHeight();
        az.spaceWidth = fm.charWidth(' ');
        
        double widthLine = az.lineWidth;
        double heightLine = az.lineHeight;
...
//                    }
     ...
     */
    public double[] getDimension(String s, Font f){
        ...
        values[0] = fm.stringWidth(s);
        values[1] = fm.getHeight();
        values[2] = fm.getAscent();
        values[3] = fm.getLeading();
        values[4] = fm.getDescent();
